...
import { FormContent } from 'components/Tab/styled';
...
const Widget = () => {
  ...
  useEffect(() => {
    ...
    const calculateDataCounts = () => {
      ...
      vacanciesData.forEach((vacancy) => {
        const vacancyName = vacancy['Informe Vacantes']?.NombreVacante;
        const vacancyCounts = {};
        ['Nuevo', 'Aceptado', 'Entrevista', 'Seleccionado', 'Descartado'].forEach((status) => {
          ...
          const filteredData = candidatesData.filter(
            (candidate) => candidate['Informe Vacantes']?.NombreVacante === vacancyName && candidate['Informe Vacantes']?.Estado === status
          );
  ...
  const handleSaveColor = () => {
    if (selectedVacancy && selectedColor && selectedCardName) {
      ...
      if (
        vacanciesData.some((vacancy) => vacancy['Informe Vacantes']?.NombreVacante === selectedVacancy) &&
              ...
              >
                <Grid container>
                  ...
                  <Grid item xs={1} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <ColorIcon
                      color={
                        selectedVacancy === vacancy['Informe Vacantes']?.NombreVacante
                          ? vacancyColors[selectedVacancy]?.[selectedCardName] || '#34D399'
                          : '#34D399'
                      }
                      onClick={() => handleColorIconClick(vacancy['Informe Vacantes']?.NombreVacante)}
                    />
                    {isPickerVisible && selectedVacancy === vacancy['Informe Vacantes']?.NombreVacante && (
                ...
                <Grid container spacing={2} justifyContent="space-around" alignItems="center" marginTop={1}>
                  <Grid item xs={2}>
                    <ProgressContainer
                      color={vacancyColors[vacancy['Informe Vacantes']?.NombreVacante]?.Nuevo || 'transparent'}
                  ...
                  <Grid item xs={2}>
                    <ProgressContainer
                      color={vacancyColors[vacancy['Informe Vacantes']?.NombreVacante]?.Aceptado || 'transparent'}
                  ...
                  <Grid item xs={2}>
                    <ProgressContainer
                      color={vacancyColors[vacancy['Informe Vacantes']?.NombreVacante]?.Entrevista || 'transparent'}
                      onClick={() => handleCardClick(vacancy['Informe Vacantes']?.NombreVacante, 'Entrevista')}
                    >
                      <ProgressBar color={vacancyColors[vacancy['Informe Vacantes']?.NombreVacante]?.Entrevista || 'transparent'} />
                  ...
                  <Grid item xs={2}>
                    <ProgressContainer
                      color={vacancyColors[vacancy['Informe Vacantes']?.NombreVacante]?.Seleccionado || 'transparent'}
                      onClick={() => handleCardClick(vacancy['Informe Vacantes']?.NombreVacante, 'Seleccionado')}
                    >
                      <ProgressBar color={vacancyColors[vacancy['Informe Vacantes']?.NombreVacante]?.Seleccionado || 'transparent'} />
                      <ProgressText>0</ProgressText>
                  ...
                  <Grid item xs={2}>
                    <ProgressContainer
                      color={vacancyColors[vacancy['Informe Vacantes']?.NombreVacante]?.Descartado || 'transparent'}
                      onClick={() => handleCardClick(vacancy['Informe Vacantes']?.NombreVacante, 'Descartado')}
                    >
                      <ProgressBar color={vacancyColors[vacancy['Informe Vacantes']?.NombreVacante]?.Descartado || 'transparent'} />
                      <ProgressText>0</ProgressText>
                      <div>Descartado</div>
                    </ProgressContainer>
                  </Grid>
