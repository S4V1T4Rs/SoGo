...
const rolRef = collection(db, 'Rol');
//fireController.js
...
export const createRol = async (
  ...
  try {
    ...
    const apiResponse = await axios.post('http://localhost:8080/api/rol', {
      ...data
    ...
    await setDoc(doc(rolRef, nextId.toString()), {
      id: nextId.toString(),
      'Detalles de Roles': rolesData['Detalles de Roles']
    });
...
export const getRol = async () => {
  try {
    const querySnapshot = await getDocs(rolRef);
    const roles = [];
    const apiResponse = await axios.get(`http://localhost:8080/api/rol`);
...
export const updateRol = async (docId, data, datafire) => {
  try {
    // Realizar la solicitud de actualización a través de Axios
    const apiResponse = await axios.put(`http://localhost:8080/api/rol/${docId}`, { ...data });
    console.log('Response from API Update:', apiResponse.data);

    // Verificar si la actualización en la API fue exitosa
    if (apiResponse.status === 200) {
      // Obtener el documento actual de Firestore
      const rolDoc = await getDoc(doc(rolRef, docId));
      if (rolDoc.exists()) {
        // Actualizar solo los campos relevantes en Firestore
        const newData = {
          'Detalles de Roles': datafire['Detalles de Roles']
        };
        await updateDoc(doc(rolRef, docId), newData);
...
export const deleteRol = async (docId) => {
  try {
    await deleteDoc(doc(rolRef, docId));
